...
export async function issueJwtTokens(userId: string, res: any, language: SupportedLanguage = SupportedLanguages.ENGLISH, dbService: DbService, jwtHelper: JwtHelper, httpService: HttpService, logger: Logger)
{
	// If valid, issue new tokens
	const tokens = await jwtHelper.generateTokens(userId);
	...
	await dbService.saveRefreshToken(userId, tokens.refresh_token, tokens.refresh_token_expires_at);
}

export async function issueVerificationToken(userId: string, email: string, language: SupportedLanguage = SupportedLanguages.ENGLISH, dbService: DbService, httpService: HttpService, logger: Logger)
{
	...
	await dbService.saveVerificationToken(userId, token, expiresAt);
	...
	while (!sent && attempts < env.EMAIL_VERIFICATION_MAX_ATTEMPTS)
	{
		...
		try
		{
			await sendEmailVerification(email, token, language, httpService);
			sent = true;
		}
		catch (error)
		{
	...
	if (!sent)
		logger.error(`Failed to send verification email after ${attempts} attempts for user ID ${userId} and email ${email}`);
}

export async function issueForgotPasswordToken(email: string, language: SupportedLanguage = SupportedLanguages.ENGLISH, dbService: DbService, httpService: HttpService, logger: Logger)
{
	...
	// Store the token in the database associated with the user (not implemented here, but should be done in a real application)
	await dbService.saveForgotPasswordToken(email, token, expiresAt);
	...
	while (!sent && attempts < env.FORGOT_PASSWORD_MAX_ATTEMPTS)
	{
		...
		try
		{
			await sendForgotPasswordEmail(email, token, language, httpService);
			sent = true;
		}
		catch (error)
		{
	...
	if (!sent)
		logger.error(`Failed to send forgot password email after ${attempts} attempts for email ${email}`);
}